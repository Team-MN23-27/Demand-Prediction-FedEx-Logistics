...
def init_db_if_needed():
    ...
    # Default demo account - login: admin / admin123
    cur.execute(
...
def get_recent_trend(hub, target_date, days=14):
    """
    Andha hub-oda last N days actual demand history return pannrom - trend chart-ku.
    """
    hub_hist = hist_df[hist_df["Hub"] == hub].sort_values("Date")
...
def login_required(f):
    """Login pannala na, login page-ku redirect pannum."""
    @wraps(f)
...
@login_required
def home():
    hub_avg_chart = {h.replace("_Hub", ""): round(v, 1) for h, v in HUB_AVG_DEMAND.items()}
    return render_template("index.html", hubs=HUBS, result=None, hub_avg_chart=hub_avg_chart)
    ...
    try:
        ...
        if not date_str or not hub:
            ...
            return render_template("index.html", hubs=HUBS, result=None,
                                    error="Please provide both Date and Hub!", hub_avg_chart=hub_avg_chart)
        ...
        return render_template("index.html", hubs=HUBS, result=result,
                                ...
                                trend=trend, hub_avg_chart=hub_avg_chart)

    except Exception as e:
        hub_avg_chart = {h.replace("_Hub", ""): round(v, 1) for h, v in HUB_AVG_DEMAND.items()}
        return render_template("index.html", hubs=HUBS, result=None,
                                error=f"Error: {str(e)}", hub_avg_chart=hub_avg_chart)
