...
def compute_gap(needed, available):
    """
    Needed vs available compare panni, shortage/surplus + suggestion kudukum.
    available None-a irundha (user kudukalana), gap analysis skip pannrom.
    """
    ...
    if gap > 0:
        status = "shortage"
        message = f"{gap} short! Extra {gap} hire/rent pannanum."
    elif gap < 0:
        status = "surplus"
        message = f"{abs(gap)} extra irukku (surplus)."
    else:
        status = "exact"
        message = "Exact-a match aaguthu, extra venaam."
...
def predict():
    try:
        ...
        # High demand check -> andha hub-oda average-ah vida jaasti irundha,
        # available workers/vehicles contact list kaamikrom
